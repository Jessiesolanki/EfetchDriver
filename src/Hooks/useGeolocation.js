...
const requestLocationPermission = async () => {
    ...
    try {
        ...
        if (granted === PermissionsAndroid.RESULTS.GRANTED) {
            ...
            Alert.alert('Notice', 'Efetch needs your location to work properly.')
...
const locationReason = async () => {
    try {
        Alert.alert('Location Access', 'Efetch is using your location to Track the orders.', [
            { text: 'Okay', onPress: () => getCurrentPosition() }
        ])
     AsyncStorage.setItem('Approval', approved)
...
const locationChecker = async () => {
    ...
    const check = await AsyncStorage.getItem('Approval')
    if (check) getCurrentPosition()
    else locationReason()
}